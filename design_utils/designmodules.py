...
def argmax_bypass(pssm):
    seq = torch.nn.functional.one_hot(torch.argmax(pssm,-1),5).float()
    tmp = (seq-pssm).detach()
    seq = pssm + tmp
    return seq
    ...
    def forward(self, msa, ss, res_id=None, num_recycle=3, msa_cutoff=500, is_training=False):
        ...
        for c in range(1 + num_recycle):
            with torch.set_grad_enabled(is_training):
                ...
                plddt = torch.einsum('bik,k->bi', plddt_prob, torch.arange(0.01, 1.01, 0.02, device=msa.device))
...
class InputEmbedderforDeisgn(nn.Module):
    
    def __init__(self, dim=48, in_dim=47):
        super(InputEmbedderforDeisgn, self).__init__()
        self.bn1 = nn.InstanceNorm2d(in_dim, affine=True)
        self.elu1 = nn.ELU(inplace=True)
        self.conv1 = nn.Conv2d(in_dim, dim, 1)
        self.linear1 = nn.Sequential(
    ...
    def forward(self, pssm, ss, msa_cutoff=500):
        msa = torch.argmax(pssm,-1)
        ...
        with torch.no_grad():
            ...
            # 1D features
            f1d_seq = pssm[0,0, :, :4]
            f1d_pssm = self.msa2pssm(pssm[0,0], w)

            f1d = torch.cat([f1d_seq, f1d_pssm], dim=1)

            # 2D features
            f2d_dca = self.fast_dca(pssm[0], w)
        ...
        pair = self.linear1(f2d.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        m = torch.einsum('he,bnlh->bnle',self.token_emb.weight.clone(), pssm[:, :msa_cutoff, :])
        return {'pair': pair, 'msa': m}
...
class DistPredictorforDesign(nn.Module):
    def __init__(self, dim_2d=48, layers_2d=12):
        super(DistPredictorforDesign, self).__init__()

        self.input_embedder = InputEmbedderforDeisgn(dim=dim_2d)
        self.recycle_embedder = RecyclingEmbedder(dim=dim_2d)
        self.net2d = RNAformer(dim=dim_2d,
                               depth=layers_2d,
                               msa_tie_row_attn=False,
    ...
    def forward(self, pssm, ss, res_id=None, num_recycle=3, msa_cutoff=500, is_training=False):
        reprs_prev = None
        msa = torch.argmax(pssm,-1)
        # msa1hot = argmax_bypass(pssm)
        msa1hot = ( torch.nn.functional.one_hot(torch.argmax(pssm,-1),5).float() - pssm).detach() + pssm
        # print('msa1hot:',msa1hot.shape)
        N, L = msa.size()[-2:]
        for c in range(1 + num_recycle):
            with torch.set_grad_enabled(is_training):
                with torch.cuda.amp.autocast(enabled=False):
                    
                    # reprs = self.input_embedder(pssm.view(1,N,L,5), ss.view(1, L, L), msa_cutoff=msa_cutoff)
                    reprs = self.input_embedder(msa1hot.unsqueeze(0), ss.view(1, L, L), msa_cutoff=msa_cutoff)
